...
When the request explicitly requires files, the Agent first stores the final artifacts on a comment created by the current task, then passes `--deliver-task-attachments`. The adapter selects only attachments whose `source_task_id` equals the current `MULTICA_TASK_ID`, downloads them, validates metadata size and SHA-256, and calls the Slack Skill configured by `slackCliPath` through `files_upload --as user`. Attachments have a separate `attempting`/`sent` receipt and are never uploaded again after an unknown result. A definite Slack Skill rejection marked `retry_safe: true` clears the intent so the same source can be retried after permissions or parameters are corrected. Limits are 20 files, 25 MiB per file, and 100 MiB total.
