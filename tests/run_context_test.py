...
class FinalReplyTests(unittest.TestCase):
    ...
    def test_issue_link_reuses_current_issue_without_extra_queries(self):
        ...
        with patch.object(final, "query", side_effect=[[self.run], agent, []]) as query:
            ...
            summary = final.summarize(data, "DEMO-87", "demo", "https://multica.example/")
            self.assertEqual(query.call_count, 3)
        self.assertEqual(summary["issue_identifier"], "DEMO-87")
        self.assertEqual(summary["issue_url"], f"https://multica.example/demo/issues/{issue_id}")
    ...
    def test_issue_link_requires_all_display_context(self):
        ...
        for args in ((None, "demo", "https://multica.example"),
                     ("DEMO-87", None, "https://multica.example"),
                     ("DEMO-87", "demo", None), (None, None, None)):
    ...
    def test_invalid_issue_context_hides_only_link(self):
        self.run["issue_id"] = "00000000-0000-4000-8000-000000000001"
        cases = [(value, "demo", "https://multica.example") for value in
                 ("#DEMO-87", "DEMO-0", "DEMO-87|<!here>", "DEMO-87\n", "", 87)]
        cases += [("DEMO-87", value, "https://multica.example") for value in
                  ("../other", "demo/issues", "demo?x=1", "", "x" * 101)]
        cases += [("DEMO-87", "demo", value) for value in
    ...
    def test_issue_link_uses_explicit_web_origin_and_workspace(self):
        ...
        summary = final.summarize(self.data, "LAB-5", "example-lab", "https://web.example:8443")
        self.assertEqual(summary["issue_url"],
                         "https://web.example:8443/example-lab/issues/00000000-0000-4000-8000-000000000001")
    ...
    def test_invalid_issue_id_never_becomes_link_path(self):
        for issue_id in ("run", "../other", "", None):
            with self.subTest(issue_id=issue_id):
                ...
                summary = final.summarize(self.data, "DEMO-87", "demo", "https://multica.example")
                self.assertNotIn("issue_url", summary)
    ...
    def test_cli_persists_issue_link_with_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            argv = ["run_context.py", "--issue", issue_id, "--issue-identifier", "DEMO-87",
                    "--workspace-slug", "demo", "--app-url", "https://multica.example", "--output", output]
            ...
            self.assertEqual(saved["issue_identifier"], "DEMO-87")
    ...
    def test_missing_link_parameters_are_resolved_in_current_scope(self):
        issue = {"id": "issue", "workspace_id": "ws", "identifier": "DEMO-99"}
        workspace = {"id": "ws", "slug": "demo"}
        ...
        with patch.object(final, "query", side_effect=[issue, workspace, config]) as query:
            self.assertEqual(final.link_context(self.data), ("DEMO-99", "demo", "https://web.example"))
        self.assertEqual(query.call_count, 3)
    ...
    def test_foreign_or_missing_link_context_is_not_used(self):
        cases = [
            [{"id": "other", "workspace_id": "ws", "identifier": "DEMO-99"},
             {"id": "other", "slug": "demo"}, "server_url: https://other.example\napp_url: https://web.example\n"],
            [{"id": "issue", "workspace_id": "other", "identifier": "DEMO-99"}, None, ""],
    ...
    def test_cli_demo99_missing_slug_and_app_url_regression(self):
        issue_id = "00000000-0000-4000-8000-000000000001"
        self.run["issue_id"] = issue_id
        workspace = {"id": "ws", "slug": "demo"}
        config = "server_url: https://multica.example\napp_url: https://web.example\n"
        with tempfile.TemporaryDirectory() as directory:
            output = str(Path(directory) / "context.json")
            argv = ["run_context.py", "--issue", issue_id, "--issue-identifier", "DEMO-99", "--output", output]
            ...
            saved = final.json.loads(Path(output).read_text())
            self.assertEqual(saved["issue_identifier"], "DEMO-99")
            self.assertEqual(saved["issue_url"], f"https://web.example/demo/issues/{issue_id}")
            self.assertIn("duration_seconds", saved["statistics"])

    def test_runtime_link_environment_does_not_read_personal_config(self):
        env = {"FINAL_REPLY_APP_URL": "https://web.example", "FINAL_REPLY_WORKSPACE_SLUG": "demo"}
        with patch.object(final, "query") as query:
            self.assertEqual(final.link_context(self.data, "DEMO-100", env=env),
                             ("DEMO-100", "demo", "https://web.example"))
            query.assert_not_called()
    ...
    def test_cli_demo100_isolated_config_uses_runtime_environment(self):
        issue_id = "00000000-0000-4000-8000-000000000001"
        self.run["issue_id"] = issue_id
        issue = {"id": issue_id, "workspace_id": "ws", "identifier": "DEMO-100"}
        with tempfile.TemporaryDirectory() as directory:
            output = str(Path(directory) / "context.json")
            env = {**self.env, "MULTICA_TASK_CONFIG_ROOT": directory,
                   "FINAL_REPLY_APP_URL": "https://web.example", "FINAL_REPLY_WORKSPACE_SLUG": "demo"}
            ...
            saved = final.json.loads(Path(output).read_text())
            self.assertEqual(saved["issue_identifier"], "DEMO-100")
            self.assertEqual(saved["issue_url"], f"https://web.example/demo/issues/{issue_id}")
