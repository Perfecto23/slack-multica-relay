...
import { buildEnvelope, messageFingerprint, compareTs, serializeEnvelope, type ThreadContext } from '../src/context-envelope.js';
...
describe('tree Slack context', () => {
  it('reads first-mention nearby threads and retains an old current root once', async () => {
    ...
    const f=vi.fn<typeof fetch>().mockImplementation(async input=>{
      ...
      if(u.pathname.endsWith('history')) {expect(u.searchParams.get('oldest')).toBe('88200.000001');return Response.json({ok:true,messages:[{...msg('89000.000001'),reply_count:2},msg('3599.000001'),{...msg('89500.000001'),thread_ts:'89000.000001'}]});}
    ...
    const envelope=JSON.parse(buildEnvelope(e,c));expect(envelope.schemaVersion).toBe(5);
  ...
  it('selects 12 preceding roots plus the current root exactly once',async()=>{
    ...
    const c=await readContext(e,'test',f);expect(c.timeline.messages).toHaveLength(13);
    expect(c.timeline.messages.filter(m=>m.ts===e.threadTs)).toHaveLength(1);expect(c.timeline.status).toBe('truncated');
  });
  it('does not widen the 30-minute window in a sparse conversation',async()=>{
    ...
    const f=vi.fn<typeof fetch>().mockImplementation(async input=>{
      ...
      expect(url.searchParams.get('oldest')).toBe('98200.000001');
    ...
    const c=await readContext(e,'test',f);
    expect(c.timeline.messages.map(m=>m.ts)).toEqual(['99500.000001',e.messageTs]);
  ...
  it('expands two recently active side branches with five replies each',async()=>{
    const e={...event,messageTs:event.threadTs};
    ...
    expect(sides).toHaveLength(2);expect(sides.flatMap(m=>m.replies?.messages??[])).toHaveLength(10);
  ...
  it('rejects missing mandatory context before dispatch',async()=>{
    ...
    await expect(readContext(event,'test',f)).rejects.toThrow('context_required_unavailable');
  });
  ...
  it('does not present an old prefix when the latest thread suffix is outside the page budget',async()=>{
    ...
    await expect(readContext(event,'test',f)).rejects.toThrow('context_required_page_limit');expect(n).toBe(10);
  });
...
describe('snapshot delivery recovery',()=>{
  it('freezes retries but refreshes the next mention after recovering a legacy issue',async()=>{
    ...
    expect(decode(comments[0]!.content).context.sinceTs).toBe(event.messageTs);
    expect(decode(comments[0]!.content).context.initialCutoffTs).toBe(event.messageTs);
  });
});
